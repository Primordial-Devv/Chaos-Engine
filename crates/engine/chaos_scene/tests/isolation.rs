...
#[test]
fn the_renderer_and_the_asset_pipeline_never_import_the_scene_system() {
    ...
    // Comme le verrou wgpu : on traque les IMPORTS et usages qualifiés,
    // pas les mentions de documentation (un commentaire n'est pas un
    // couplage).
    let leaks: Vec<PathBuf> = sources
        .into_iter()
        .filter(|file| {
            let source = fs::read_to_string(file).expect("source illisible");
            source.contains("use chaos_scene") || source.contains("chaos_scene::")
        })
        .collect();

    assert!(
        leaks.is_empty(),
        "chaos_scene fuit dans le renderer ou l'Asset Pipeline : {leaks:?}"
    );
}

#[test]
fn the_scene_dependency_stays_out_of_renderer_and_assets_manifests() {
    let root = workspace_root();
    let manifests = [
    ...
    let offenders: Vec<&PathBuf> = manifests
        ...
        .filter(|manifest| {
            ...
            content
                .lines()
                .any(|line| line.trim_start().starts_with("chaos_scene"))
        })
        .collect();

    assert!(
        offenders.is_empty(),
        "chaos_scene déclaré dans un manifeste interdit : {offenders:?}"
    );
}

#[test]
fn the_scene_system_only_knows_the_core_and_the_ecs() {
    let root = workspace_root();
    let manifest = root.join("crates/engine/chaos_scene/Cargo.toml");
    let content = fs::read_to_string(&manifest).expect("manifeste illisible");
    ...
    let foreign: Vec<&str> = content
        ...
        .map(str::trim_start)
        .filter(|line| {
            line.starts_with("chaos_")
                && !line.starts_with("chaos_core")
                && !line.starts_with("chaos_ecs")
        })
        .collect();

    assert!(
        foreign.is_empty(),
        "chaos_scene dépend d'autres crates que chaos_core et chaos_ecs : {foreign:?}"
    );
}
