...
- `architecture/engine-loop.md` — **l'Engine Core, mature V1** : cycle de vie, modèle de frame, temps variable/fixe, subsystems ordonnés, services du contexte, configuration validée, headless, modèle d'erreurs, diagnostics/metrics, interruptions, garanties d'arrêt, frontière d'application.
- `architecture/threading.md` — la politique de threads : ce qui reste main thread à vie, les contrats `Send`/`Sync` verrouillés à la compilation, les deux futurs préparés (render thread, systèmes parallèles) sans un seul thread lancé.
