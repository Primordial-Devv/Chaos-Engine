...
- **`Time` est la première ressource réelle** — le « temps global » (rejointe par `FixedTime`, le pas fixe des simulations déterministes). La frontière : `Time` (l'instantané, données pures) entre dans le monde ; `FrameClock` (la machinerie qui possède l'`Instant` et le clamp) reste au moteur. La donnée entre dans l'ECS, la machinerie jamais. Le branchement moteur → resource `Time` à chaque frame viendra avec la sous-phase d'intégration.
...
- **La politique des phases vit au moteur** : `chaos_engine::stages::{UPDATE, LATE_UPDATE, POST_UPDATE}` — simulation, mises à jour tardives, propagation — déclarés au démarrage dans cet ordre garanti (le modèle d'exécution complet : `docs/architecture/engine-loop.md`).
